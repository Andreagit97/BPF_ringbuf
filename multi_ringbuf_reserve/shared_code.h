...
#include <errno.h>
#include <signal.h>
#include "multi_ringbuf_reserve.skel.h"
#include <stdint.h>
...
static struct multi_ringbuf_reserve_bpf *skel = NULL;
...
static void clean()
{
	...
	multi_ringbuf_reserve_bpf__destroy(skel);
}
...
static bool open_skeleton()
{
	...
	skel = multi_ringbuf_reserve_bpf__open();
	if(!skel)
	{
...
{
	int err = multi_ringbuf_reserve_bpf__load(skel);
	if(err)
	{
...
{
	int err = multi_ringbuf_reserve_bpf__attach(skel);
	if(err)
	{
